...
def main():
    ...
    for i in range(2, sheet.max_row+1):
        protocol_name = sheet[f'A{i}'].value  # 框架协议名称
        contract_number = sheet[f'B{i}'].value  # 框架协议编号
        contract_sign_date = datetime.datetime.strptime(
            sheet[f'C{i}'].value, '%Y-%m').strftime('%Y年%m月')  # 框架协议签订年月
        project_name = sheet[f'D{i}'].value  # 工程名称
        project_address = sheet[f'E{i}'].value  # 工程地址
        party_a_name = sheet[f'F{i}'].value  # 甲方名称
        party_b_name = sheet[f'G{i}'].value  # 乙方名称
        start_date = datetime.datetime.strptime(
            sheet[f'H{i}'].value, '%Y-%m-%d').strftime('%Y年%m月%d日')  # 开工日期
        completion_date = datetime.datetime.strptime(
            sheet[f'I{i}'].value, '%Y-%m-%d').strftime('%Y年%m月%d日')  # 竣工日期
        warranty_period = sheet[f'J{i}'].value  # 质保期
        settlement_sign_date = datetime.datetime.strptime(
            sheet[f'K{i}'].value, '%Y-%m').strftime('%Y年%m月')  # 结算协议签订年月
        settlement_amount = round(sheet[f'L{i}'].value, 2)  # 结算金额
        settlement_amount_capital = num_to_capital(settlement_amount)  # 结算金额大写
        tax_rate = sheet[f'M{i}'].value  # 税率
        tax_rate_percent = f'{tax_rate * 100:.0f}%'  # 税率百分比
        settlement_amount_untax = round(settlement_amount / (1 + tax_rate),
                                        2)  # 结算不含税金额
        settlement_amount_tax = round(settlement_amount - settlement_amount_untax,
                                    2)  # 结算税金
        paid_amount = round(sheet[f'N{i}'].value, 2)  # 已支付金额
        paid_amount_capital = num_to_capital(paid_amount)  # 已支付金额大写
        balance_amount = round(settlement_amount - paid_amount, 2)  # 剩余金额
        balance_amount_capital = num_to_capital(balance_amount)  # 剩余金额大写
        settlement_amount_untax_balance = round(balance_amount / (1 + tax_rate),
                                                2)  #剩余不含税金额
        settlement_amount_tax_balance = round(
            balance_amount - settlement_amount_untax_balance, 2)  # 剩余税金
        payment_ratio = sheet[f'O{i}'].value  # 结算后支付比例
        payment_ratio_percent = f'{payment_ratio * 100:.0f}%'  # 结算后支付比例百分比
        payment_amount = round(settlement_amount * payment_ratio, 2)  # 结算后应支付金额
        payment_amount_capital = num_to_capital(payment_amount)  # 结算后应支付金额大写
        payment_amount_this = round(payment_amount - paid_amount, 2)  # 本次付款金额
        payment_amount_this_capital = num_to_capital(payment_amount_this)  # 本次付款金额大写
        warranty_ratio = sheet[f'P{i}'].value  # 质保金比例
        warranty_ratio_percent = f'{warranty_ratio * 100:.0f}%'  # 质保金比例百分比
        warranty_amount = round(settlement_amount * warranty_ratio, 2)  # 质保金金额
        warranty_amount_capital = num_to_capital(warranty_amount)  # 质保金金额大写
